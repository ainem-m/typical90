...
if __name__ == "__main__":
    ...
    B = list(acc(A))
    # i回目の操作で∑Ai移動する
    # i回目の操作での最大値はmax(B[:i+1])
    # sumのセグ木とmaxのセグ木持てばいけそう
    max_seg = SegmentTree(monoid_list["max"], n)
    max_seg.build(B)
    sum_seg = SegmentTree(monoid_list["sum"], n)
    sum_seg.build(A)
    cur: Union[int, float] = 0
    ans: Union[int, float] = 0
    for i in range(n):
        ans = max(ans, cur + max_seg.get(0, i + 1))
        cur += sum_seg.get(0, i + 1)
    print(ans)
